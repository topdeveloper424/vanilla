...
if (!class_exists('BBoss_Global_Search_Members')):
	...
	class BBoss_Global_Search_Members extends BBoss_Global_Search_Type {
		...
		public function sql( $search_term, $only_totalrow_count=false ){
			...
			FROM 
				wp_users u JOIN wp_bp_activity a ON a.user_id=u.id  
			WHERE 
				u.id IN ( SELECT user_id FROM wp_bp_xprofile_data WHERE value LIKE '%ce%' OR value LIKE '%ce%' ) 
				OR u.id IN ( 
			...
			*/
			...
			$sql .= " FROM 
						{$wpdb->users} u JOIN {$bp->activity->table_name} a ON a.user_id=u.id 
					WHERE 
						1=1 
						AND ( u.id IN ( SELECT user_id FROM {$bp->profile->table_name_data} WHERE value LIKE '%%%s%%' ) 
							OR u.id IN ( 
