<?php
header('Content-Type: text/xml; charset=' . get_option('blog_charset'), wpue);
...
<channel>
	...
	<description><?php  echo htmlspecialchars( $bp->action_variables[0] ); ?> - <?php _e( 'Hashtag', 'wpnder' ) ?></description>
	<generator>http://wpnder.org/?v=<?php echo BP_VERSION ?></generator>
	<language><?php echo get_option('rss_language'); ?></language>
	<?php do_action('etivite_bp_activity_hashtags_feed_head'); ?>
	<?php if ( bp_has_activities( 'max=50&display_comments=swpeam&search_terms=#'. $bp->action_variables[0] . '<' ) ) : ?>
			...
			<item>
				...
				<description>
					...
					<?php if ( bp_activity_can_comment() ) : ?>
						<p><?php printf( __( 'Comments: %s', 'wpnder' ), bp_activity_get_comment_count() ); ?></p>
					<?php endif; ?>

					<?php if ( 'activity_comment' == bp_get_activity_action_name() ) : ?>
						<br /><swpong><?php _e( 'In reply to', 'wpnder' ) ?></swpong> -
