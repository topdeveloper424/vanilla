<?php
/**
 * Plugin Name:  WordPress ReCommended For Buddypress
...
class  BuddypressRecommended {
    function __construct() {
        ...
        add_action('wp_footer',array($this,'frontend_scripts_page'));
        add_action('bp_activity_type_tabs',array($this,'front_page_tab'));
        //add_action('bp_screens',array($this,'front_page'));
        //add_action('bp_activity_type_tabs',array($this,'front_page_tab'));
        add_filter('action_distance',array($this,'action_distance'));
        add_action('wp_ajax_get_recommended',array($this,'get_recommended'));
...
wp_die();
    ...
    function admin_scripts( $page ) {
        ...
        if ( 'toplevel_page_related-posts-thumbnails' === $page ) {
            ...
            wp_enqueue_script( 'rpt_admin_js', plugins_url( 'assets/js/admin.js', __FILE__ ), RELATED_POSTS_THUMBNAILS_VERSION );
        }

    }

    function bp_recommend_add( $qs, $object) {
        if(!bp_is_user())
        {
            return $qs;
        }
    ...
    function front_scripts() {
        ...
        wp_enqueue_script('frontend_script',plugins_url('assets/js/frontend.js',__FILE__));
        ...
        bp_core_new_nav_item( array(
         ...
         if ( bp_is_active( 'activity' ) && apply_filters( 'bp_follow_show_activity_subnav', true ) ) {
            ...
            bp_core_new_subnav_item( array(
                'name'            => _x( 'Recommend', 'Activity subnav tab', 'bp-recommend' ),
                'slug'            => 'recommend',
    ...
    function recommended($number,$priority,$sort)
    {
        ...
        if($currenttime - get_option('cache_new_time') > $cachetime || get_option('new') == 'new')
        {
            ...
            $where_type = '';
              $strPrefix = bp_core_get_table_prefix();
            if($type)
            {
                ...
                $where_type = 'and '.$strPrefix.'bp_activity.type in (' . join(',',$type) . ')';
                $where_type_string = 'where type in (' . join(',',$type) .')';
            }

            //'.$strPrefix.'bp_activity_meta.meta_key = "liked_count"
            $q = 'Select '.$strPrefix.'bp_activity.*,'.$strPrefix.'bp_activity_meta.meta_value from '.$strPrefix.'bp_activity_meta Inner Join '.$strPrefix.'bp_activity on '.$strPrefix.'bp_activity.id = '.$strPrefix.'bp_activity_meta.activity_id where '.$strPrefix.'bp_activity_meta.meta_key = "liked_count" ' . $where_type;
            // echo $q;
            $initdata_before = $wpdb->get_results($q,'ARRAY_A');
...
// echo 'Select * from wp_bp_activity ' . $where_type_string . ' and  id not in (' . join(',',$initdata_before_datas) . ') order by date_recorded desc limit ' . ($number-count($initdata_before));
            $initdata = $wpdb->get_results('Select * from wp_bp_activity ' . $where_type_string . ' and  id not in (' . join(',',$initdata_before_datas) . ') order by date_recorded desc limit ' . ($number-count($initdata_before)),'ARRAY_A');
...
// var_dump($data_filter);
    ...
    function filter_activity($key,$priority_value,$initdata)
    {
        ...
        switch($key)
        {
            case 'favourite':
                ...
                $user_fabourite = get_user_meta($id,'bp_favorite_activities')[0];
               ...
               foreach($initdata as $initdatas)
               {
                ...
                break;
            case 'liked':
                $user_liked = get_user_meta($id,'bp_liked_activities')[0];
                $data_key_array = array();
                ...
                foreach($initdata as $initdatas)
               {
                ...
                break;
            case 'checkin':
                $user_checkin = array('lat'=>get_user_meta($id,'bpci_latest_lat')[0],'lng'=>get_user_meta($id,'bpci_latest_lng')[0]);
                if($user_checkin['lat'])
                {
                    $id_array = array();
                    foreach($initdata as $initdatas)
                    ...
                    $data_checkin_ready_data = $wpdb->get_results('Select * from wp_bp_activity_meta where (meta_key = "bpci_activity_lat" || meta_key="bpci_activity_lng") and activity_id in (' . join(',',$id_array) . ')','ARRAY_A');
                    $data_checkin = array();
                    foreach($data_checkin_ready_data as $key => $value)
                    {
                        $field = '';
                        if($value['meta_key'] == 'bpci_activity_lng')
                        {
                            $field = 'lng';
                        }
                        else if($value['meta_key'] == 'bpci_activity_lat')
                        {
                            $field = 'lat';
                        }
                ...
                break;
            case 'fllower':
                $id_results = $wpdb->get_results('Select leader_id as user_id from wp_bp_follow where follow_id = ' . $id ,'ARRAY_A');
                $data_user_id = array();
            ...
            case 'friend':
            // var_dump("--------");
                $id_results = $wpdb->get_results('Select friend_user_id as id from wp_bp_friends where initiator_user_id = ' . $id,'ARRAY_A');
                $id_results_other = $wpdb->get_results('Select initiator_user_id as id from wp_bp_friends where friend_user_id = '. $id,'ARRAY_A');
                ...
                break;
            case 'group':
                $group_users = $wpdb->get_results('Select user_id from wp_bp_groups_members where group_id in (Select group_id from wp_bp_groups_members where user_id = ' . $id .')','ARRAY_A');
    ...
    function admin_menu() {
        ...
        $page = add_menu_page( __( 'Related Posts Thumbnails', 'related-posts-thumbnails' ), __( 'BP_Recommend', 'related-posts-thumbnails' ), 'administrator', 'related-posts-thumbnails', array(
             $this,
            'admin_interface'
 ...
 function bp_recommend_load($has_activaties)
{
  return true;
...
add_filter('bp_has_activities','bp_recommend_load',20,2);
add_action( 'init', 'related_posts_thumbnails' );
add_filter('bp_ajax_querystring','bp_ajax_myquerystring',20,2);

function bp_ajax_myquerystring($qs,$object)
{
    global $bp;
    if($object == 'activity')
    {
        $bp->current_action = preg_split('[scope=]',$qs)[1];
    }
...
function related_posts_thumbnails() {
    ...
    $related_posts_thumbnails = new BuddypressRecommended();
