	...
	function iwmp_add_single_script() { ?>
		...
		jQuery(document).ready(function($) {
			...
			function imoc_init(){
				...
				$.magnificPopup.page = ( $.cookie('bp-activity-oldestpage') * 1 );
				$.magnificPopup.current_page = ( $.cookie('bp-activity-oldestpage') * 1 );
...
//asamir add try and catch to prevent js error that prevent privacy with media to work
			...
			$('body > ul> li > #activity_media_filter > input[type="checkbox"]').change(function(){
				window.setTimeout(imoc_init(),1000);
			});

			$('.item-list-tabs ul li').click(function(){
				window.setTimeout(imoc_init(),1000);
			});
	});
