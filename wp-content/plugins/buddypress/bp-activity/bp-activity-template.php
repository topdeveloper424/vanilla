...
/***
	...
	if ( bp_is_current_action( bp_get_activity_slug() ) ){
		...
		// $include = (!$include && IsNumber(bp_is_current_action( bp_get_activity_slug() )) )? bp_is_current_action( bp_get_activity_slug() ) : false ;
		// echo $include;
		}

		$tempargs = wp_parse_args($args);
					...
					$display_comments = 'stream';
						...
						if($scope == 'friends'){
							...
							if(class_exists('BP_Follow')){
 							$following_list = BP_Follow::get_following($user_id );
						...
						if(defined('FP_POSTS_SAVE_VIEWS_NUMBER') && FP_POSTS_SAVE_VIEWS_NUMBER == '1'){
							// var_dump("enabled");
									if(isset($GLOBALS['featured_post']) ){
										$fp_promote_views = bp_activity_get_meta( $GLOBALS['featured_post'] , 'fp_promote_views' , true );
										if(empty($fp_promote_views))
										$fp_promote_views = 0;
						        bp_activity_update_meta( $GLOBALS['featured_post'] , 'fp_promote_views' , (int)$fp_promote_views+1 );
									}
						}
				 }
		}
