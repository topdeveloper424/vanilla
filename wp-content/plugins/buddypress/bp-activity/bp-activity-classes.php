...
// echo "<b>".$select_sql.$from_sql.$where_sql.$sort. $pag_sql."</b><hr>";
		...
		if(isset($GLOBALS['featured_post'])){
			...
			$pfactivities = $wpdb->get_results( $select_sql. " ". $from_sql . " WHERE ". $pfwhere_sql );
...
// echo  '<b>'.bp_is_active( 'xprofile' ).'</b>';
...
if(strpos($filter_array['object'],',') == false){
	if ( !empty( $filter_array['activities_ids'] ) ) {
		...
		$activities_ids_sql = BP_Activity_Activity::get_in_operator_sql( 'a.id', $filter_array['activities_ids'] );
