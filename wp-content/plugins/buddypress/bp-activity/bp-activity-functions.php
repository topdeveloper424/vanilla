		...
		if( isset($_POST['period_in_min'])){
			$period_in_min = preg_replace( "/\D/" , "" , $_POST['period_in_min'] );
					if(is_numeric($period_in_min) && $period_in_min != 0)
					{
					 ...
					 bp_activity_update_meta( $activity->id , 'fp_promote' , $fp_promote_details );
					 bp_activity_update_meta( $activity->id , 'fp_promote_Expire' , (int)$fp_promote_details["createdat"]+(int)$fp_promote_details["expire_in"] );
					 bp_activity_update_meta( $activity->id , 'fp_promote_type' , 'ad' );
				}
	}
	return $activity->id;
}
...
function get_featured_post(){
		global $bp, $wpdb;
		return $wpdb->get_var( $wpdb->prepare( "SELECT a.id
					FROM wp_bp_activity a
					inner join wp_bp_activity_meta m on m.activity_id = a.id
					WHERE m.meta_key = 'fp_promote_Expire' and m.meta_value > unix_timestamp() ORDER BY RAND() limit 1" ) );
}
function get_All_featured_post(){
		global $bp, $wpdb;
		return $wpdb->get_results( $wpdb->prepare( "SELECT a.id,(m.meta_value - unix_timestamp()) as expire_in,m.meta_value as expire_date,IFNULL(m2.meta_value,0) views,m3.meta_value type
						FROM wp_bp_activity a
						inner join wp_bp_activity_meta m on m.activity_id = a.id and  m.meta_key = 'fp_promote_Expire'
						left outer join wp_bp_activity_meta m2 on m2.activity_id = a.id and m2.meta_key = 'fp_promote_views'
						left outer join wp_bp_activity_meta m3 on m3.activity_id = a.id and m3.meta_key = 'fp_promote_type'
