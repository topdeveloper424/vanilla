...
// var_dump($img);
		...
		<a href="<?php echo esc_url(bpfb_get_image_url($activity_blog_id) . trim($img)); ?>" class="<?php echo $use_thickbox; ?>" rel="<?php echo $rel;?>" <?php echo $target; ?> >
			<?php if(IsVideoFile($img)){ ?>
			  <video width="320" height="240" controls><source  src="<?php echo esc_url(bpfb_get_image_url($activity_blog_id) . trim($img)); ?>" type="video/mp4"></video>
			<?php } else {
				 ?>
			<img src="<?php if($thumbnail =='') echo esc_url(bpfb_get_image_url($activity_blog_id) . trim($img)); else  echo esc_url($thumbnail);?>"   />
<?php } ?>
		</a>
