...
$(function() {
	...
	// 		 }
 	// 			bp_activity_request(scope,param);
	// });
...
var $textContainer;
		...
		$.post(ajaxurl, {"action":"bpfb_preview_remote_image", "data":imgs}, function (data) {
			...
			$.each(data, function(k,file) {
				...
				// 		var html = "";
						 ...
						 html += '<img class="bpfb_preview_photo_item" src="' + file + '" width="80px" />';
						html += '<input type="hidden" class="bpfb_photos_to_add" name="bpfb_photos[]" value="' + file + '" />';
		...
		//
	 	...
	 	var html = "";
			if(fileName.endsWith('mp4'))
			 html += '<video width="320" height="240" controls><source  src="' + _bpfb_data.temp_img_url + resp.file + '" type="video/mp4"></video>';
			 else
			 html += '<img class="bpfb_preview_photo_item" src="' + _bpfb_data.temp_img_url + resp.file + '" width="80px" />';
			html += '<input type="hidden" class="bpfb_photos_to_add" name="bpfb_photos[]" value="' + resp.file + '" />';
	...
	var removeTempImages = function (rti_callback) {
		...
		$.post(ajaxurl, {"action":"wp_ajax_bpfb_remove_temp_images", "data": $imgs.serialize().replace(/%5B%5D/g, '[]')}, function (data) {
			rti_callback();
		});
	};
...
var BpfbVideoHandler = function () {
	...
	var createMarkup = function () {
		// var html = '<div class="video-scope" width=50%><input type="text" id="bpfb_video_url" name="bpfb_video_url" placeholder="' + l10nBpfb.paste_video_url + '" value="" />' +
		// 	'<input type="button" id="bpfb_video_url_preview" value="' + l10nBpfb.preview + '" /></div>';
			var html =	'<div class="video-scope"  width=50%><input type="button" id="bpfb_add_remote_video" value="' + l10nBpfb.add_remote_video + '" /><div id="bpfb_remote_video_container"></div>' +
				'<input type="button" id="bpfb_remote_video_preview" value="' + l10nBpfb.preview + '" /></div>';

			html +=
			'<div class="photo-scope"  width=50%><input type="button" id="bpfb_add_remote_image" value="' + l10nBpfb.add_remote_image + '" /><div id="bpfb_remote_image_container"></div>' +
			'<input type="button" id="bpfb_remote_image_preview" value="' + l10nBpfb.preview + '" /></div>';
		$container.empty().append(html);
		$("#bpfb_remote_image_preview").hide();
		$("#bpfb_remote_video_preview").hide();
		$(window).off("resize.bpfb").on("resize.bpfb", resize);
		resize();
		//asamir add handle remote video
		$("#bpfb_add_remote_video").click(function () {
			$('.video-scope').show();
			$('.photo-scope').hide();
			$("#bpfb_remote_image_preview").hide();
			$("#bpfb_remote_video_preview").show();


			// $("#bpfb_add_remote_video").val(l10nBpfb.add_another_remote_image);

			$("#bpfb_remote_video_container").append(
				'<input type="text" id="bpfb_video_url" name="bpfb_video_url" placeholder="' + l10nBpfb.paste_video_url + '" value="" />'
			);
	...
	var processForSave = function () {
		if($("#bpfb_video_url").val()){
				return {
					"bpfb_video_url": $("#bpfb_video_url").val()
				};
		}else if($('input.bpfb_photos_to_add').length && $('input.bpfb_photos_to_add').length >0){
			var $imgs = $('input.bpfb_photos_to_add');
			var imgArr = [];
			$imgs.each(function () {
				imgArr[imgArr.length] = $(this).val();
